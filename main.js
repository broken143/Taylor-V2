  ...
  const importPromises = lodash.map(LibsFiles, async file => {
    ...
    try {
      ...
      if (err) {
        ...
        const module = await (await import(file))?.default || await import(file);
...
async function watchFiles() {
  try {
    const pluginsDir = path.resolve(directoryName, "plugins");
    const libsDir = path.resolve(directoryName, "lib");
    const mainDir = path.resolve(directoryName, "");
    console.log(`Plugins Dir: ${pluginsDir}`);
    console.log(`Libs Dir: ${libsDir}`);
    console.log(`Main Dir: ${mainDir}`);
    const watcher = chokidar.watch([pluginsDir, libsDir, mainDir], {
      ignored: (path, stats) => stats?.isFile() && !path.includes("node_modules") && !path.endsWith(".js"),
      persistent: true,
      ignoreInitial: true,
      alwaysState: true
    });
    watcher.on("add", async filePath => {
      ...
      try {
        const dir = filePath.startsWith(pluginsDir) ? "plugins" : filePath.startsWith(libsDir) ? "lib" : "main";
        ...
        console.error(`Error handling 'add' event for '${filePath}': ${e.message}`);
        logger.error(`\nError handling 'add' event for '${filePath}': ${e.message}`);
      }
      ...
      try {
        ...
        console.error(`Error handling 'change' event for '${filePath}': ${e.message}`);
        logger.error(`\nError handling 'change' event for '${filePath}': ${e.message}`);
      }
      ...
      try {
        ...
        console.error(`Error handling 'unlink' event for '${filePath}': ${e.message}`);
        logger.error(`\nError handling 'unlink' event for '${filePath}': ${e.message}`);
      }
    }).on("error", error => {
    ...
    logger.error(`\nError watching files: ${e.message}`);
  }
}
